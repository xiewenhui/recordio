...
::google::protobuf::int64 File::ModifyTime() {
  struct stat st;
  if (lstat(name_.c_str(), &st) != 0) {
    return 0;
  }
...
char* File::ReadLine(char* const output, ::google::protobuf::uint64 max_length) {
  return fgets(output, max_length, f_);
}

::google::protobuf::int64 File::ReadToString(std::string* const output, ::google::protobuf::uint64 max_length) {
  CHECK_NOTNULL(output);
  output->clear();

  if (max_length == 0) return 0;

  ::google::protobuf::int64 needed = max_length;
  int bufsize = (needed < (2 << 20) ? needed : (2 << 20));

  boost::scoped_array<char> buf(new char[bufsize]);

  ::google::protobuf::int64 nread = 0;
  while (needed > 0) {
  ...
  return (nread >= 0 ? static_cast< ::google::protobuf::int64>(output->size()) : -1);
}
...
util::Status GetContents(
  ...
  if (file != NULL) {
    const ::google::protobuf::int64 size = file->Size();
